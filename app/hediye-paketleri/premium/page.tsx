...
type Bottle = {
  ...
  title: string;
  desc: string;
  priceDelta: number; // istersen şişeye göre fiyat farkı
...
const BOTTLES: Bottle[] = [
  {
    id: "bottle_gold",
    title: "Blue Gold",
    desc: "Lüks görünüm, premium kapak.",
    priceDelta: 0,
    image: "/gift-bottles/standard-bottle.png",
  },
  {
    id: "bottle_black",
    title: "White Onyx",
    desc: "Modern, güçlü ve sade.",
    priceDelta: 0,
    image: "/gift-bottles/standard-bottle-1 .png", // ✅ dosya adında boşluk var (senin klasördeki isim gibi)
  },
  {
    id: "bottle_crystal",
    title: "Dragon Scale",
    desc: "Daha özel, vitrinlik hissiyat.",
    priceDelta: 750,
    image: "/gift-bottles/standard-bottle-1.png", // ✅ aynı şişelerden kullan
  },
];

export default function PremiumHediyePaketi() {
  ...
  const toggle = (name: string) => {
    setSelected((prev) => {
      ...
      if (prev.length >= MAX_SELECT) {
        showToast({ title: "Limit dolu", desc: `En fazla ${MAX_SELECT} esans seçebilirsin.`, tone: "warn" });
        return prev;
      }
  ...
  const totalPrice = useMemo(() => PRICE_PREMIUM + (bottle?.priceDelta || 0), [bottle]);
  ...
  const progress = useMemo(() => {
    ...
    // 2 adım: şişe + esans (premium ağırlık)
    return Math.round(((b * 0.35 + s * 0.65) * 100 + Number.EPSILON) * 100) / 100;
  }, [selected.length, bottleId]);

  const addToCart = () => {
    if (!canContinue) {
      if (!bottleId) showToast({ title: "Önce şişeni seç", desc: "Premium pakette şişe seçimi zorunludur.", tone: "warn" });
      else showToast({ title: "Seçim tamamlanmadı", desc: `Tam ${MAX_SELECT} esans seçmelisin.`, tone: "warn" });
      return;
    }

    const item = {
      id: "gift_prm_" + Date.now(),
      createdAt: Date.now(),

      // ✅ Sepet sayfasının doğru okuması için:
      kind: "gift",
      tier: "premium",
      title: "Premium Paket",

      // (mevcut alanlarını bozma)
      type: "hediye-premium",

      // ✅ Sepet sayfası bottle.name bekliyor -> object gönderiyoruz
      bottle: bottle ? { id: bottle.id, name: bottle.title } : undefined,

      // (mevcut alanını bozma)
      bottleId,
    ...
    setJustAdded(true);
    showToast({ title: "Sepete eklendi", desc: "Premium Paket hazır.", tone: "ok" });
  ...
  const resetAll = () => {
    setBottleId("");
    setSelected([]);
    showToast({ title: "Seçimler sıfırlandı", desc: "Yeni bir premium kombinasyon oluşturabilirsin.", tone: "info" });
  };
    ...
    <main style={page}>
      ...
      <div style={wrap}>
        <header style={hero}>
          ...
          <h1 style={title}>Premium Hediye Paketi</h1>
          <p style={subtitle}>
            Önce <b>şişeni seç</b>, sonra kutuya girecek <b>{MAX_SELECT} esansı</b> belirle. Premium paket; daha zengin
            harman, daha güçlü iz ve daha rafine bir deneyim için tasarlandı.
          </p>
          ...
          <div style={progressWrap}>
            <div style={progressTop}>
              <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <span style={progressMeta}>
                  ...
                  • Esans:{" "}
                  <b style={{ color: selected.length === MAX_SELECT ? "#d4af37" : "rgba(255,255,255,0.7)" }}>
            ...
            <div style={progressTrack} aria-label="İlerleme Çubuğu">
              <div style={{ ...progressFill, width: `${Math.max(6, progress)}%` }} />
            </div>
          </div>
        </header>
        ...
        <section style={card}>
          <div style={sectionHeader}>
            ...
            <div style={sectionHint}>Premium görünüm • Zorunlu</div>
          </div>
            ...
            {BOTTLES.map((b) => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...bottleCard,
                    borderColor: active ? "rgba(212,175,55,0.85)" : "rgba(212,175,55,0.26)",
                    background: active ? "linear-gradient(180deg, rgba(212,175,55,0.18), rgba(0,0,0,0.34))" : glass,
                    transform: active ? "translateY(-2px)" : "translateY(0)",
                    boxShadow: active ? "0 22px 60px rgba(0,0,0,0.58)" : "0 18px 48px rgba(0,0,0,0.46)",
                  }}
                >
                  <div style={bottleTop}>
                    <div style={{ display: "grid", gap: 4 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "baseline" }}>
                        ...
                        {b.priceDelta > 0 ? (
                          <div style={deltaPill}>+{b.priceDelta.toLocaleString("tr-TR")} ₺</div>
                        ) : (
                          <div style={deltaPillMuted}>Dahil</div>
                        )}
                      </div>

                      <div style={{ opacity: 0.78, fontSize: 13, lineHeight: 1.45 }}>{b.desc}</div>
                    </div>
          ...
          <div style={noteLine}>
            <span style={noteDot} />
            Premium pakette şişe seçimi zorunludur.
          </div>
        </section>
        ...
        <section style={card}>
          <div style={sectionHeader}>
            ...
            <div style={sectionHint}>Tam {MAX_SELECT} seçim • Rafine harman</div>
          </div>
          ...
          <div style={metaRow}>
            ...
            <div style={helperText}>
              Premium profil önerisi: <b>2–3</b> üst nota, <b>4–5</b> orta nota, <b>3–4</b> baz nota.
            </div>
          </div>
            ...
            {ESANSLAR.map((e) => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...essence,
                    borderColor: active ? "rgba(212,175,55,0.85)" : "rgba(212,175,55,0.22)",
                    background: active ? "rgba(212,175,55,0.16)" : "rgba(0,0,0,0.28)",
          ...
          <div style={noteLine}>
            <span style={noteDot} />
            {MAX_SELECT}’den fazla seçemezsin. İstersen seçtiklerini tekrar tıklayıp kaldırabilirsin.
          </div>
        </section>
        ...
        <div style={footer}>
          ...
          <div style={footerRight}>
            <button
              onClick={() => {
                if (!bottleId) showToast({ title: "Şişe seçmeden ilerleyemezsin", desc: "Önce şişeni seç.", tone: "warn" });
                else showToast({ title: "Devam için tamamla", desc: `Tam ${MAX_SELECT} esans seçmelisin.`, tone: "info" });
              }}
              style={{ ...secondaryBtn, opacity: canContinue ? 0.7 : 1 }}
        ...
        >
          ...
          >
            <div style={toastTop}>
              ...
              <button onClick={() => setToast((p) => ({ ...p, open: false }))} style={toastClose} aria-label="Bildirimi kapat">
                ✕
              </button>
            </div>
...
const deltaPillMuted: React.CSSProperties = {
  padding: "7px 10px",
  borderRadius: 999,
...
const ctaSpark: React.CSSProperties = {
  ...
  background: "rgba(0,0,0,0.22)",
  boxShadow:
    "0 0 0 5px rgba(0,0,0,0.08), 0 0 24px rgba(212,175,55,0.55), 0 0 60px rgba(212,175,55,0.25)",
};
...
const successPulse: React.CSSProperties = {
  ...
  inset: 0,
  pointerEvents: "none",
  background: "radial-gradient(900px 320px at 80% 80%, rgba(212,175,55,0.12), rgba(0,0,0,0) 60%)",
...
if (typeof document !== "undefined") {
  const id = "anka_prm_gift_keyframes";
